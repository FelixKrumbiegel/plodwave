...
def main(save_path: str) -> None:
    ...
    with open(f'{save_path}/H1errors_plod_th12.csv', 'w') as file:
        file.write('H')
        for ell in range(ellmin_p, ellmax_p+1):
            for p in range(pmin, pmax+1):
    ...
    with open(f'{save_path}/L2errors_plod_th12.csv', 'w') as file:
        file.write('H')
        for ell in range(ellmin_p, ellmax_p+1):
            for p in range(pmin, pmax+1):
                file.write(f',p{p}ell{ell}')
        ...
        for ell in range(ellmin_p, ellmax_p+1):
            for p in range(pmin, pmax+1):
                ...
                L2error[H-Hmin, ell-ellmin_p, p-pmin] = np.sqrt(
                    ...
                    np.dot(uref, mesh.cg_mass_fine.dot(uref)))
                print(f'H={H}, ell={ell}, p={p}, '
                      + f'L2error={L2error[H-Hmin, ell-ellmin_p, p-pmin]}')

    with open(f'{save_path}/H1errors_plod_th4.csv', 'w') as file:
        file.write('H')
        for ell in range(ellmin_p, ellmax_p+1):
            for p in range(pmin, pmax+1):
                file.write(f',p{p}ell{ell}')
    ...
    with open(f'{save_path}/L2errors_plod_th4.csv', 'w') as file:
        file.write('H')
        for ell in range(ellmin_p, ellmax_p+1):
            for p in range(pmin, pmax+1):
                file.write(f',p{p}ell{ell}')
