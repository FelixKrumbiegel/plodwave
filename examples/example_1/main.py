...
def main(save_path: str) -> None:
    ...
    H1error = np.zeros((Hmax-Hmin+1, ellmax-ellmin+1, pmax-pmin+1))
    L2error = np.zeros((Hmax-Hmin+1, ellmax-ellmin+1, pmax-pmin+1))
    ...
    with open(f'{save_path}/H1errors_plod.csv', 'w') as file:
        ...
        for H in range(Hmin, Hmax+1):
            ...
            for ell in range(ellmin, ellmax+1):
                for p in range(pmin, pmax+1):
                    file.write(f',{H1error[H-Hmin, ell-ellmin, p-pmin]}')
    ...
    with open(f'{save_path}/L2errors_plod.csv', 'w') as file:
        ...
        for H in range(Hmin, Hmax+1):
            ...
            for ell in range(ellmin, ellmax+1):
                for p in range(pmin, pmax+1):
                    file.write(f',{L2error[H-Hmin, ell-ellmin, p-pmin]}')
    return
